...
- (void)setTopic:(BSJTopic *)topic
{
    ...
    CGFloat contentTextHeight = [topic.text boundingRectWithSize:aSize options:NSStringDrawingUsesLineFragmentOrigin attributes:@{NSFontAttributeName : AdaptedFontSize(14)} context:nil].size.height;
    ...
    if (topic.type != BSJTopicTypeWords) {
        if (topic.height !=0 && topic.width !=0) {
...
//            [self.topic.topCmts addObject:self.topic.topCmts.firstObject];
        ...
        _cellHeight += self.topCmtLayout.textBoundingSize.height + 20 + 10 +16;
    }
...
- (void)handleUIData
{
    self.zanCount = [self countFormat:self.topic.ding placeholder:@"顶"];
    
    self.caiCount = [self countFormat:self.topic.cai placeholder:@"踩"];
    
    self.repostCount = [self countFormat:self.topic.repost placeholder:@"分享"];
    
    self.commentCount = [self countFormat:self.topic.comment placeholder:@"评论"];
...
- (NSString *)formatCreatTime
{
    ...
    //    NSDateComponents *cmps = [NSDate dateFromStringDate:_created_at withDateStrFormat:SUPDateStringFormat toDate:[NSDate date]];
...
- (NSString *)countFormat:(NSInteger)count placeholder:(NSString *)placeholder
{
    if (count > 10000) {
